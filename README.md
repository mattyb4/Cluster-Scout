...
1. **Filter** — merges and filters the PTMD and TCGA datasets. A mutation must show up a minimum of 3 times for it to be added to the filtered dataset. This variable can be changed by changing HOTSPOT_MIN_AFFECTED_CASES near the top of scripts/1_filter.py
...
1. **Filter** — filters the TCGA dataset for recurrent hotspot mutations and maps gene names to UniProt IDs
...
The main output for PTM-proximity mode is **`Output/ptm_mutation_proximity_db.tsv`** — a table of PTM sites, their nearby TCGA mutations, 3D distances, and HTP/LTP annotations.
